...
/// <summary>
/// Source: http://weblogs.sqlteam.com/mladenp/archive/2008/02/11/Watershed-Image-Segmentation-in-C.aspx
/// </summary>
namespace Segmentation
{
    ...
    public class WatershedGrayscale
    {
        ...
        private void CreatePixelMapAndHeightSortedArray(ref Vector3[,] data)
        {
            _pictureWidth = data.GetLength(0);
            _pictureHeight = data.GetLength(1);
            ...
            for (int x = 0; x < _pictureWidth; x++)
            {
                for (int y = 0; y < _pictureHeight; y++)
                {
                    ...
                    _heightSortedList[height].Add(p);
                }
            }
        ...
        private void DrawWatershedLines(ref Vector3[,] data)
        {
            ...
            for (int x = 0; x < _pictureWidth; x++)
            {
                for (int y = 0; y < _pictureHeight; y++)
                {
                    if (_pixelMap[x.ToString() + "," + y.ToString()].Label == WatershedCommon.WSHED)
                        data[x,y] = new Vector3(watershedColor, watershedColor, watershedColor);
                }
            }
        }

        public void ProcessFilter(ref Vector3[,] imageData)
        {
            CreatePixelMapAndHeightSortedArray(ref imageData);
            Segment();
            DrawWatershedLines(ref imageData);
        }
    }
